...
from hri.Utils import equal, pool, get_unifs, gumbel_softmax_sample, map_rules_to_pred, fuzzy_and, merge
from hri.Infer import infer_one_step_vectorise, infer_one_step, infer_one_step_campero, infer_tgt_vectorise, infer_one_step_vectorise_neo
from hri.Evaluate import evaluation
...
# -----------------------------------------------------
    ...
    def train(self, task=None):
        ...
        if self.args.train_on_original_data:
        ...
        for epoch in range(self.args.num_iters):
            ...
            valuation_init, bg_pred_ind_ls_noTF, target, tgt_mask, num_constants, _ = self.data_generator.getData(
                mode='train')
            ...
            # pdb.set_trace()
            # Note: No closed world assumtion here, target=0 means no info, not False.
            valuation_tgt *= tgt_mask
            target *= tgt_mask
            ...
            if self.args.stop_training == "threshold":
                ...
                if ave_loss < self.args.training_threshold and ave_loss > 0:
                    ...
                    eval_num_iters = task.data_generator.dataset.len_val_file_ids
                    if self.args.get_PR:
                ...
                if epoch % self.args.eval_interval == 0:
                    if self.args.save_ckpt:
                        assert self.args.ckpt_path 
                        os.makedirs(self.args.ckpt_path, exist_ok=True)
                        model_path = os.path.join(self.args.ckpt_path, 'iter_{}.pth'.format(epoch))
