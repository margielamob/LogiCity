...
from .Utils import pool, top_k_top_p_sampling, get_unifs, gumbel_softmax_sample, map_rules_to_pred, fuzzy_and, fuzzy_or, merge
...
def extract_symbolic_path(unifs, full_rules_str, predicates_labels=None):
    """
    ...
    while len(queue)>0:
        ...
        #1---get symbolic son
        if predicate ==tgt_idx:#for unified model, only look body1 in tgt
    ...
    symbolic_formula=get_symbolic_formula(symbolic_path, num_predicates, full_rules_str, predicates_labels=predicates_labels)
    
    return symbolic_path, symbolic_formula, symbolic_unifs, rule_max_body_idx

def get_symbolic_formula(symbolic_path, num_predicates, full_rules_str, predicates_labels=None):
    """
    ...
    if predicates_labels==None:
        predicates_labels = ["init_"+str(i) for i in range(0, num_background-2)]
