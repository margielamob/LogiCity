...
class Z3PlannerExpert(Z3Planner):
    def __init__(self, yaml_path):        
        super().__init__(yaml_path)

    def _create_rules(self):
        assert "Task" in self.data["Rules"].keys(), "Make sure the task rule is defined, it is used to evaluate and collect reward"
        assert "Sim" in self.data["Rules"].keys(), "Make sure the sim rule is defined, it is used to control the deterministic agents' behavior"
        assert "Expert" in self.data["Rules"].keys(), "Make sure the expert rule is defined, it is used to roll out expert agents' traj"
        self.rules = {
            "Task": {},
            "Sim": {},
            "Expert": {}
