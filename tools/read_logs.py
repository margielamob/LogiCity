...
run = 2
log_dir = 'log_rl/tl_nlm_val{}'.format(run)
checkpoints_template = 'checkpoints/logicity_transfer_medium{run}_tl{data}/checkpoints/checkpoint_{checkpoint}.pth'
...
for data in data_values:
    for checkpoint in checkpoints:
        log_filename = f'transfer_easy2med_nlm_val_{checkpoint}_{run}_{data}.log'
...
for data, checkpoints_scores in results.items():
    ...
    for checkpoint in sorted_checkpoints:
        checkpoint_path = checkpoints_template.format(run=run, data=data, checkpoint=checkpoint)
        selected_checkpoints.append(checkpoint_path)

# Write the shell script to a file
with open('scripts/rl/test_all_nlm{}.sh'.format(run), 'w') as f:
    f.write('checkpoints=(\n')
    ...
    f.write('  data=$(echo $c | grep -o "medium{}_tl[0-9]*" | grep -o "[0-9]*$")\n'.format(run))
    f.write('  python3 main.py --config config/tasks/Nav/transfer/medium/algo/nlmtest.yaml --exp transfer_easy2med_nlm_test_${iter}_2_${data} --checkpoint_path $c --use_gym\n')
    f.write('done\n')
